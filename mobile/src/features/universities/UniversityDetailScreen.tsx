    ...
    <Screen scroll style={styles.screen}>
      <View style={[styles.hero, shadows.premium]}>
        ...
        <Pressable style={styles.favoriteFloating} onPress={handleFavoritePress}>
          <SvgIcon name={data.is_favorite ? 'heartFilled' : 'heart'} size={24} color={colors.accent} />
        </Pressable>
          ...
          <View style={styles.programsList}>
            {data.programs.map(program => (
              <AppCard key={program.id} style={styles.programCard}>
                ...
                <ProgramMeta icon="money" text={`Стоимость: ${program.tuition_fee ? `${program.tuition_fee} ${program.currency || ''}` : 'уточняется'}`} />
              </AppCard>
            ))}
          </View>
        </>
...
function InfoCard({ icon, label, value }: { icon: SvgIconName; label: string; value: string }) {
  return (
    <AppCard style={styles.infoCard}>
      <View style={styles.infoIconBox}><SvgIcon name={icon} size={21} color={colors.primary} /></View>
...
const styles = StyleSheet.create({
  ...
  favoriteFloating: { position: 'absolute', top: spacing.lg, right: spacing.lg, width: 50, height: 50, borderRadius: 25, backgroundColor: colors.card, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.border },
  ...
  infoIconBox: { width: 48, height: 48, borderRadius: 18, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.sm },
  ...
  markdownBox: { marginTop: spacing.xl },
