...
export function ProgramDetailScreen() {
  ...
  const query = useQuery({ queryKey: ['program-detail', route.params.id], queryFn: () => educationCatalogApi.getProgram(route.params.id) });

  if (query.isLoading) return <Loading />;
  if (query.isError || !query.data) return <Screen scroll style={styles.screen}><ErrorState onAction={() => query.refetch()} /></Screen>;
  ...
  return (
    <Screen scroll style={styles.screen}>
      ...
      {program.requirements ? <AppCard style={styles.card}><Text style={styles.cardTitle}>Требования</Text><Text style={styles.text}>{program.requirements}</Text></AppCard> : null}
      <AppButton title="Подать заявку" onPress={() => navigation.navigate('ApplicationCreate', { universityId: program.university, programId: program.id })} style={styles.button} />
